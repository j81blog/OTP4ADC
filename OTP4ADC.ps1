...
.EXAMPLE
    ...
    Use a csv with the following header
    "Username","DeviceName"

    If you want to set your own secrets, you can use the following header
    "Username","DeviceName","Secret"

    Default Delimiter is a comma ",", specify the -Delimiter parameter de define a different one
...
Machine: $($Env:COMPUTERNAME) ($([Environment]::OSVersion.VersionString))
PSCulture: $($PSCulture)
        ...
        if ($WriteLog) {
            ...
            if ($Show) {
                ...
                try {
                    ...
                    [IO.File]::AppendAllText($LogFile, $LogString, [Text.Encoding]::Unicode)
...
"@
    ...
    if ($GeneratedQR.Width -ne $Width) {
        ...
        $NewQR = [Drawing.Graphics]::FromImage($Bitmap)
...
function Get-OTPToken {
    <#
        .SYNOPSIS
            Generate the token for a given secret
    #>
    [CmdletBinding()]
    param (
        [Parameter(Mandatory = $true)]
...
function Initialize-GUI {
    ...
    $SyncHash.Form.WindowStartupLocation = [Windows.WindowStartupLocation]::CenterScreen
    #Set GUI with value
    $SyncHash.WPFControl_tbAttribute.Text = $SyncedVariables.Attribute
    #Reset OTPDevices & DeviceSecrets variable
...
function Update-Gui {
    ...
    try { $SyncHash.App.Dispatcher.Invoke([Windows.Threading.DispatcherPriority]::Background, [action] { }) } catch { <# nothing #> }
...
function Reset-GUIQRImage {
    <#
        .SYNOPSIS
            Reset the GUI QR Image section to the default / initial values
    #>
...
function Get-GUIQRImage {
    <#
        .SYNOPSIS
            Prepare the GUI for loading a QR code
    #>
...
function Show-QR {
    <#
        .SYNOPSIS
            Set GUI text and set focus to QR
    #>
...
function Invoke-SettingsChangedQuestion {
    ...
    $result = [Windows.MessageBox]::Show("There are unsaved settings!`nDo you want to continue and loose the made changes?", "Unsaved Settings", [Windows.MessageBoxButton]::YesNo, [Windows.MessageBoxImage]::Exclamation)
    $output = $false
...
function Reset-GUIUser {
    ...
    if (-Not $SyncedVariables.CleanGUIUser) {
        if ($SyncedVariables.Saved) {
            ...
            $result = [Windows.MessageBox]::Show("There are unsaved changes!`nDo you want to continue and loose the made changes?", "Unsaved changes", [Windows.MessageBoxButton]::YesNo, [Windows.MessageBoxImage]::Exclamation)
            switch ($result.ToString()) {
                "Yes" { $Continue = $true }
                "No" { $Continue = $false }
...
function Reset-GUIOTPToken {
    ...
    try { $PoSH.EndInvoke($SyncedVariables.handle) } catch { <# nothing #> }
...
function Set-GUISettingsEncryptionOptionEnabled {
    [CmdLetBinding()]
    param(
        [Bool]$IsEnabled = $true
    )
...
function Save-OtpToUserExportCommand {
    ...
    $result = [Windows.MessageBox]::Show("The PowerShell command to make the necessary changes was copied to the clipboard.`nClean the current screen? Changes are not saved to the selected user unless you run the copied command!", "PowerShell Command", [Windows.MessageBoxButton]::YesNo, [Windows.MessageBoxImage]::Question)
...
function Invoke-SearchADUser {
    ...
    if ([String]::IsNullOrEmpty($($SyncHash.WPFControl_tbUsername.Text))) {
        $null = [Windows.MessageBox]::Show("The Username field is empty!", "Username Empty", [Windows.MessageBoxButton]::OK, [Windows.MessageBoxImage]::Error)
    } else {
...
function Import-QRModule {
    ...
    try {
        ...
        $null = [Reflection.Assembly]::Load([Convert]::FromBase64String($content))
        ...
        $null = [Windows.MessageBox]::Show("Could not load the `"QRCodeGenerator`"!`nQR Code generation is disabled.", "QRCodeGenerator Failed", [Windows.MessageBoxButton]::OK, [Windows.MessageBoxImage]::Exclamation)
    }
...
function Import-ModulesGUI {
    ...
    if ($Script:LDAPNativeFunctions) {
        ...
        try {
            ...
            if ($SyncHash.WPFControl_gbUser.IsEnabled) { $SyncHash.WPFControl_gbUser.IsEnabled = $false }
            $null = [Windows.MessageBox]::Show("Error while loading the `"ActiveDirectory Module`"!`r`nYou can try to set the (experimental) `"Native AD`" option under settings.", "ActiveDirectory Module", [Windows.MessageBoxButton]::OK, [Windows.MessageBoxImage]::Error)
        }
    } else {
        $SyncHash.WPFControl_tbUsername.Text = "ActiveDirectory Module NOT Found!"
        if ($SyncHash.WPFControl_gbUser.IsEnabled) { $SyncHash.WPFControl_gbUser.IsEnabled = $false }
        $null = [Windows.MessageBox]::Show("The PowerShell Module `"ActiveDirectory`" was NOT Found!`r`nYou can try to set the (experimental) `"Native AD`" option under settings.", "ActiveDirectory Module", [Windows.MessageBoxButton]::OK, [Windows.MessageBoxImage]::Error)
    }
    Import-QRModule
...
function Import-GUIAppImage {
    ...
    $AppImage.BeginInit()
    $AppImage.StreamSource = [IO.MemoryStream][Convert]::FromBase64String($AppImageB64)
    $AppImage.EndInit()
    $AppImage.Freeze()
...
function Import-GUISettings {
    ...
    if ([String]::IsNullOrEmpty($SyncedVariables.Settings.LDAPSettings.LDAPNativeFunctions)) {
        ...
        $SyncHash.WPFControl_cbLDAPAlternativeModule.IsChecked = $true
        $Script:LDAPNativeFunctions = $true
    } else {
    ...
    if ([String]::IsNullOrEmpty($SyncedVariables.Settings.LDAPSettings.SecretEncryptionCertificateThumbprint)) {
        ...
        Set-GUISettingsEncryptionOptionEnabled -IsEnabled $false
    } else {
        ...
        Set-GUISettingsEncryptionOptionEnabled -IsEnabled $true
    }
    ...
    if ($SyncedVariables.Settings.LDAPSettings.EncryptionEnabled) {
        ...
        try {
            ...
            if ($CertTest -ne $true) {
                $null = [Windows.MessageBox]::Show("No valid certificate found with thumbprint:`r`n`"$($SyncedVariables.Settings.LDAPSettings.SecretEncryptionCertificateThumbprint)`"", "Error", [Windows.MessageBoxButton]::OK, [Windows.MessageBoxImage]::Error)
            }
        } catch {
            $null = [Windows.MessageBox]::Show("$($_.Exception.Message)", "Error", [Windows.MessageBoxButton]::OK, [Windows.MessageBoxImage]::Error)
            $SyncHash.WPFControl_cbLDAPSecretEncryptionEnabled.IsChecked = $false
...
function Stop-GUIApplication {
    [CmdletBinding()]
    param ()
    Write-Verbose "Starting function : Stop-GUIApplication"
    try {
...
function Get-ExceptionDetails {
    ...
    if ($Summary) {
        try { [void]$ErrorLines.AppendLine($($Exception | Format-List * -Force | Out-String).Trim()) } catch { <# nothing #> }
    } else {
...
function Get-AdsiADDomain {
    ...
    if (($Credential -eq [Management.Automation.PSCredential]::Empty) -and ($SyncedVariables.LDAPCredential -is [Management.Automation.PSCredential])) {
...
function Get-CertificateFromStore {
    ...
    Write-Verbose "Starting function : Get-CertificateFromStore"
    $result = Get-ChildItem -Path "Cert:\LocalMachine\My", "Cert:\CurrentUser\My" | Where-Object { $_.Thumbprint -like $Thumbprint } | Select-Object -First 1
    Write-Verbose "Ending function   : Get-CertificateFromStore"
    Write-Output $result
}
...
function Get-HashSha1 {
    ...
    if ($PsCmdlet.ParameterSetName -eq 'CertFile') {
        $cert = [Security.Cryptography.X509Certificates.X509Certificate]::new($certPath)
    }
    if ($PsCmdlet.ParameterSetName -eq 'CertThumbprint') {
        $certificate = Get-CertificateFromStore -Thumbprint $Thumbprint
        $cert = [Security.Cryptography.X509Certificates.X509Certificate]::new($certificate)
    }
...
function Test-CertificatePresent {
    ...
    if (-not [String]::IsNullOrEmpty($Thumbprint)) {
        ...
        if ([String]::IsNullOrEmpty($certificate.SerialNumber)) {
            $ErrorMessage = "No valid certificate found, SerialNumber is empty! Correct thumbprint configured?"
        } elseif (-Not ($certificate -is [Security.Cryptography.X509Certificates.X509Certificate2])) {
            $ErrorMessage = "The found certificate is not valid!"
        } elseif (-not $certificate.HasPrivateKey) {
            $ErrorMessage = "No private key found!"
        } elseif ($certificate.PrivateKey.Key.ExportPolicy -match "AllowExport" -or $certificate.PrivateKey.Key.ExportPolicy -match "AllowPlaintextExport") {
            $ErrorMessage = "Certificate looks valid but is not exportable!"
        } else {
            try {
                ...
                $ErrorMessage = "Encryption test failed. Could not read private key!`r`n$($_Exception.Message)"
            }
        }
    } else {
...
function Invoke-StartEncryption {
    ...
    if ($SyncedVariables.EncryptionOperation -eq "0" -and [String]::IsNullOrEmpty($SecretEncryptionCertificateThumbprint) ) {
        ...
        $null = [Windows.MessageBox]::Show("Please make sure a (new) certificate Thumbprint is configured!", "Encryption Operation", [Windows.MessageBoxButton]::OK, [Windows.MessageBoxImage]::Error)
        ...
        $null = [Windows.MessageBox]::Show("Please make sure a (current) certificate Thumbprint is configured!", "Encryption Operation", [Windows.MessageBoxButton]::OK, [Windows.MessageBoxImage]::Error)
        ...
        $null = [Windows.MessageBox]::Show("Please make sure a (current) attribute is configured!", "Encryption Option", [Windows.MessageBoxButton]::OK, [Windows.MessageBoxImage]::Error)
        ...
        $null = [Windows.MessageBox]::Show("Please make sure a (new) attribute is configured!", "Encryption Option", [Windows.MessageBoxButton]::OK, [Windows.MessageBoxImage]::Error)
    } else {
        $IsReady = $true
    }
    if (-Not $IsReady) {
        Write-ToLogFile -E -C OTP4ADC-Encryption -M "Cannot continue, something not ready!"
        $null = [Windows.MessageBox]::Show("Cannot continue, something not ready!", "Encryption", [Windows.MessageBoxButton]::OK, [Windows.MessageBoxImage]::Error)
    } else {
    ...
    if (-Not [String]::IsNullOrEmpty($SecretEncryptionCertificateThumbprint)) {
        $result = [Windows.MessageBox]::Show("Do you want to use the new certificate thumbprint as default?", "New default", [Windows.MessageBoxButton]::YesNo, [Windows.MessageBoxImage]::Question)
...
"@
    ...
    $SyncHash.Form.add_Closing({
            param(
                ...
                [Parameter(Mandatory)][ComponentModel.CancelEventArgs]$e
            )
    ...
    $SyncHash.WPFControl_tbSecretEncryptionCertificateThumbprint.Add_TextChanged({
            ...
            try { $this.Text = [String]($this.Text).ToUpper() } catch { <# nothing #> }
    ...
    $SyncHash.WPFControl_lvUsernames.add_SelectionChanged({ 
            ...
            if (-Not [String]::IsNullOrEmpty($($SelectedItem.Attribute))) {
                ...
                if ((Test-ValidEncryptedJson $SelectedItem.Attribute) -and ((-Not $SyncedVariables.Settings.LDAPSettings.EncryptionEnabled) -or ([String]::IsNullOrEmpty(($SyncedVariables.Settings.LDAPSettings.SecretEncryptionCertificateThumbprint))))) {
                    $null = [Windows.MessageBox]::Show("It looks like the source secret is encrypted.`r`nMake sure encryption is enabled and a correct Thumbprint is configured!", "Encrypted Token Found", [Windows.MessageBoxButton]::OK, [Windows.MessageBoxImage]::Exclamation)
                    ...
                    try {
                        ...
                        $null = [Windows.MessageBox]::Show("An error occured while decrypting the secret(s).`r`nMake sure a correct Thumbprint is configured!`r`n$($_.Encryption.Message)", "Error", [Windows.MessageBoxButton]::OK, [Windows.MessageBoxImage]::Exclamation)
                    }
                ...
                $SyncHash.WPFControl_lvOtps.ItemsSource = $null
                $SyncedVariables.DeviceSecrets = @()
                if ($SyncHash.WPFControl_tbDeviceName.IsEnabled -and $SyncedVariables.Settings.ChangeSecretDisabled) { $SyncHash.WPFControl_tbDeviceName.IsEnabled = $false }
            }
    ...
    $SyncHash.WPFControl_btnGenerateQR.Add_Click({
            ...
            if ($SyncedVariables.QRGenerationPossible) {
                ...
                $null = [Windows.MessageBox]::Show("The PowerShell Module `"QRCodeGenerator`" was NOT Found!`nQR Code generation is disabled.", "QRCodeGenerator Module", [Windows.MessageBoxButton]::OK, [Windows.MessageBoxImage]::Exclamation)
            }
            Update-Gui
        }
    )
    ...
    $SyncHash.WPFControl_btnExportQR.Add_Click({
            ...
            try {
                $SelectedItem = $SyncHash.WPFControl_lvUsernames.SelectedItem
                $PNGPath = Show-SaveFileDialog -FileName $("{0}_{1}_{2}.png" -f $SelectedItem.SamAccountName, $SyncHash.WPFControl_tbDeviceName.text, $SyncHash.WPFControl_tbGateway.text)
                Write-Verbose "PNGPath: $PNGPath"
                if (Test-Path (Split-Path -Path $PNGPath -Parent | Resolve-Path).Path) {
                    [IO.File]::WriteAllBytes($PNGPath, $($SyncedVariables.QRImage.ToArray()))
    ...
    $SyncHash.WPFControl_btnViewTOTPToken.Add_Click({
            ...
            try {
                ...
                try { $SyncedVariables.OTPToken.OTP | clip.exe } catch { <# nothing #> }
                $SyncedVariables.OTPUpdate = $true
    ...
    $SyncHash.WPFControl_btnTestSettings.Add_Click({
            ...
            try {
                ...
                if (-Not (Test-AdsiADConnection -Server $SyncHash.WPFControl_tbLDAPServer.Text -Port $SyncHash.WPFControl_tbLDAPPort.Text -Credential $Credential)) {
                    $null = [Windows.MessageBox]::Show("LDAP settings test failed!", "Test", [Windows.MessageBoxButton]::OK, [Windows.MessageBoxImage]::Error)
                    $failed = $true
                }
                ...
                if ([String]::IsNullOrEmpty($($SyncHash.WPFControl_tbSecretEncryptionCertificateThumbprint.Text)) -and ($SyncHash.WPFControl_cbLDAPSecretEncryptionEnabled.IsChecked -eq $true)) {
                    ...
                    try {
                        ...
                        Write-Verbose "ERROR: Could not perform certificate test, Error message: $($_.Exception.Message))"
                    }
                    if ($CertTest -ne $true) {
                        $null = [Windows.MessageBox]::Show("Certificate test failed!`r`n$CertTest", "Error", [Windows.MessageBoxButton]::OK, [Windows.MessageBoxImage]::Error)
                        $failed = $true
                    }
                }
                if (-Not $failed) {
                    $null = [Windows.MessageBox]::Show("Test OK!", "Test", [Windows.MessageBoxButton]::OK, [Windows.MessageBoxImage]::Information)
                }
    ...
    $SyncHash.WPFControl_btnLogPathPath.Add_Click({
            ...
            $SyncHash.WPFControl_tbLogPathPath.Text = Show-SaveFileDialog -FileName "EncryptionResults.txt" -Filter "txt files (*.txt)|*.txt|All files (*.*)|*.*"
            Update-Gui
        }
    )

    #TokenText Handler
    [Windows.RoutedEventHandler]$Script:TokenTextSelectedEventHandler = {
    ...
    $SyncHash.WPFControl_spTokenText.AddHandler(
        [Windows.Controls.RadioButton]::CheckedEvent, $Script:TokenTextSelectedEventHandler
    )

    #Settings EncryptionEnabled Handler
    [Windows.RoutedEventHandler]$Script:EncryptionEnabledCheckedEventHandler = {
        param(
            [Parameter(Mandatory)][Object]$sender
        )
    ...
    $SyncHash.WPFControl_cbChangeSecretDisabled.Add_Checked({
            ...
            # check to detect settings changes
    ...
    [Windows.RoutedEventHandler]$Script:EncryptionOperationSelectedEventHandler = {
        ...
        Write-Verbose "Result: $($SyncedVariables.EncryptionOperation)"
    }

    $SyncHash.WPFControl_spEncryptionOperation.AddHandler(
        # Add checked event
        [Windows.Controls.RadioButton]::CheckedEvent, $Script:EncryptionOperationSelectedEventHandler
    )

    #EncryptionOption Handler
    [Windows.RoutedEventHandler]$Script:EncryptionOptionSelectedEventHandler = {
        ...
        Write-Verbose "Result: $($SyncedVariables.EncryptionOption)"
    }

    $SyncHash.WPFControl_spEncryptionOption.AddHandler(
        # Add checked event
        [Windows.Controls.RadioButton]::CheckedEvent, $Script:EncryptionOptionSelectedEventHandler
    )
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC6MHgqD9fkzVq9
# Ytzq6cODNNQWB1JZhycS1FXmB/wXpaCCHl4wggTzMIID26ADAgECAhAsJ03zZBC0
...
# hkiG9w0BCQQxIgQgKZjNTAcJYxnLfv3qCMFJGOiDUSEIRY+MpLhkmB6AeiAwDQYJ
# KoZIhvcNAQEBBQAEggEAcmnU8dCmJtK2EAS24GZDvwMhkDmBtq48YrnKYyGUzcfF
# bpbiPPSwd0M6eHKdij1+ZsePgs5MczE2oyIQAm7HbcxVZrY/t/RB0/7hSPL/fqQP
# hdNJK39Na5/xYLZM4v0TMdBudhIK8Un2FyBPG8SjJmhQH+FRlAUPFZ3qS+mzEvCj
# hbYVmfoU0LAkjJv7r2O7zQGeBXSfgGU0I+AHbm9q13v7idm8TsLgtnFwgE1b/Axk
# Ehz+bHweqTbkfVuexc2K54t4HTarjBdgLSGkDRRzkhJ5U0/xBcE8AoNXqcNVFJdl
# GwfstXXS5xwcZUjGuIYaH7ZG9h1ol7QEZ+UF0GcVOaGCA0swggNHBgkqhkiG9w0B
# CQYxggM4MIIDNAIBATCBkTB9MQswCQYDVQQGEwJHQjEbMBkGA1UECBMSR3JlYXRl
# ciBNYW5jaGVzdGVyMRAwDgYDVQQHEwdTYWxmb3JkMRgwFgYDVQQKEw9TZWN0aWdv
# IExpbWl0ZWQxJTAjBgNVBAMTHFNlY3RpZ28gUlNBIFRpbWUgU3RhbXBpbmcgQ0EC
# EDlMJeF8oG0nqGXiO9kdItQwDQYJYIZIAWUDBAICBQCgeTAYBgkqhkiG9w0BCQMx
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzA5MTIyMDM3MjVaMD8GCSqG
# SIb3DQEJBDEyBDCiZKeEKuoPFeCifeVcSTj8r9C0PR08XveGdNikf1c/ut6V+1yA
# VJCZszGPtzsASTgwDQYJKoZIhvcNAQEBBQAEggIAOi2pD5UlJEiso21Jqn9VD6+A
# mb2ri9rnwI84vf3GzWv1f9p3BNDGFZFBLO/wyeWD+UoLDStq/abYsFKjCTyvV6Wt
# WQfGXz5kQOSF87wX2Rqb6xQWWRZUb/bx0lBmVY2/4wG+pHqnisKMS4oSLUVkII0r
# x70oN+4gIPRdfIt291fYQVbdCEZbM3qCFew/JIuEe87gYmJzFLPNoZ5SDObSvE5Y
# ljeiEBI6qzDYAvBgMASxIcDeZ0Av0lxrm+KyBgjqVoahGLJaUEHBCIDxCa5YeKfn
# cruscYLpOVjeSpiTyU1YGSSeqOIh0wb+pMpVlu+9P6skwUze1/nZ2Iv2vexHPSWu
# OMtlBlnc6LQp36LaQvBnyivnaiTFjfNkEZiCeXd1hJHKHmNGcc2wTVQmQMXV9EZ1
# evM1rM+gngWMc+AZNlZ80CdkQ/bQdsqVYiKsdFbSJ25JxBWZ6xvyb2vgydwyr94/
# eGoNbz+MwANX1fd1ZHPA9JvptzKu6r7O4kVijUWt0V4WnWJHct7Uy1K5sl1vx72T
# /pRFMnbEykHXli47QVQ21U/A4mslyibtauXHHgBAvlos10LapMX/O17PLu7mh2l1
# mHMUTHXxgiXTL6VDc5fl685zh2kj88fZTIKmGUeu38H5z3GdWg2TemKgNsbij/1c
# yh80nFpp8rWLHb4AOgo=
